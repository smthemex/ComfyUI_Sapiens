...
class Keypoints308_3PODataset(PoseBaseDataset):
    METAINFO: dict = dict(
    ...
    def _register_airstore_handler(self) -> None:
        ...
        self.path_template = (
            "airstoreds://codec_avatar_sapiens_3po_ala_v1_train_no_user_data"
        )
        self.airstore = True
    ...
    def get_data_info(self, idx):
        ...
        try:
            ...
            keypoints_list = pickle.loads(
                self._read_from_airstore(
                    "annotations", data_info["airstore_id"]
                ).getvalue()
            )  # shape 3 x 344
        ...
        for keypoints_info in keypoints_list:
            if keypoints_info["person_id"] == data_info["person_id"]:
                keypoints_np = np.array(keypoints_info["keypoints_2d"])  # 70 x 3
                break

        if keypoints_np is None:
            return None

        ## convert to 344 keypoints
        keypoints_344_np = np.zeros((344, 3))
        keypoints_344_np[:70] = keypoints_np  ## 344 x 3
        keypoints_344_np = keypoints_344_np.reshape(1, -1, 3)  # shape 1 x 344 x 3
        ...
        # process keypoints
        keypoints = keypoints_344_np[:, :, :2]  # shape 1 x 344 x 2
        keypoints_visible = keypoints_344_np[:, :, 2] > 0  # shape 1 x 344
