...
class NormalRandomScale(BaseTransform):
    def __init__(
        self,
        scale_min: float = 0.5,
        scale_max: float = 2.0,
    ...
    def transform(self, results: dict) -> dict:
        ...
        if "gt_normal" in results:
            normal_resized = cv2.resize(
                results["gt_normal"],
                (new_w, new_h),
                interpolation=cv2.INTER_NEAREST,
            )
            results["gt_normal"] = normal_resized
...
@TRANSFORMS.register_module()
class NormalRandomCrop(BaseTransform):
...
@TRANSFORMS.register_module()
class NormalRandomCropContinuous(BaseTransform):
    def __init__(
        self,
...
class NormalResize(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if "gt_normal" in results and self.test_mode is False:
            normal_resized = cv2.resize(
                results["gt_normal"],
                (new_width, new_height),
                interpolation=cv2.INTER_NEAREST,
            )
            final_normal = np.zeros(
                (self.target_height, self.target_width, 3), dtype=normal_resized.dtype
            )
            final_normal[
                offset_y : offset_y + new_height, offset_x : offset_x + new_width, :
            ] = normal_resized
            results["gt_normal"] = final_normal

        return results


@TRANSFORMS.register_module()
class NormalRandomFlip(BaseTransform):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if "gt_normal" in results:
            ...
            gt_normal = cv2.flip(gt_normal, 1)  # 1 for horizontal flip
            gt_normal[:, :, 0] = -gt_normal[:, :, 0]  # flip x
            results["gt_normal"] = gt_normal
...
@TRANSFORMS.register_module()
class NormalGenerateTarget(BaseTransform):
    def __init__(self, background_val: int = -1000):
        self.background_val = background_val
        return

    def transform(self, results: dict) -> dict:
        if "gt_normal" not in results.keys():
            return results

        gt_normal = results["gt_normal"]
        mask = results["mask"]
        norms = np.linalg.norm(gt_normal, axis=2, keepdims=True)
        gt_normal = gt_normal / (norms + 1e-6)
        gt_normal[mask == 0] = self.background_val
        results["gt_normal"] = gt_normal
        return results
...
@TRANSFORMS.register_module()
class NormalPackInputs(BaseTransform):
    def __init__(
        self,
    ...
    def transform(self, results: dict) -> dict:
        ...
        if "gt_normal" in results:
            ...
            gt_normal = results["gt_normal"].astype(np.float32)  ## H x W x 3
            gt_normal = gt_normal.transpose(2, 0, 1)  # H x W x 3 -> 3 x H x W
            data_sample["gt_normal"] = to_tensor(gt_normal.copy())
...
@TRANSFORMS.register_module()
class NormalResizePadImage(BaseTransform):
    def __init__(
        self,
        height: int = 1024,
        width: int = 768,
